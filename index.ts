...
export const setCredentials = ({ APIKEY, CLIENT_CODE, CLIENT_PIN, CLIENT_TOTP_PIN }: CREDENTIALS) => {
  if (!credentails) {
    credentails = {
      APIKEY,
      CLIENT_CODE,
...
export const generateSmartSession = async (credentails: CREDENTIALS): Promise<ISmartApiData> => {
  if (smartSession) {
    return smartSession;
  } else {
...
export const getMarginDetails = async (): Promise<MarginAPIResponseType | null> => {
  if (smartSession) {
...
export const delay = ({ milliSeconds }: delayType) => {
  ...
  if (milliSeconds && typeof milliSeconds === "number") delayInMilliseconds = milliSeconds;
  else if (milliSeconds && typeof milliSeconds === "string") delayInMilliseconds = parseInt(milliSeconds);
...
export const getPositions = async (smartSession: ISmartApiData, cred: CREDENTIALS) => {
  let config = {
    method: "get",
    url: GET_POSITIONS,
    headers: {
...
export const isCurrentTimeGreater = ({ hours, minutes }: TimeComparisonType): boolean => {
  const currentTime = moment().tz("Asia/Kolkata");
  const targetTime = moment().tz("Asia/Kolkata").set({ hours, minutes, seconds: 0 });
  ...
  if (scriptName && isArray(scripMaster) && scripMaster.length > 0) {
    ...
    let filteredScrip = scripMaster.filter((scrip) => {
      ...
      return (
        ...
        _get(scrip, "exch_seg") === "NFO" &&
        (_get(scrip, "instrumenttype") === "FUTSTK" || _get(scrip, "instrumenttype") === "FUTIDX") &&
        _get(scrip, "expiry") === _expiry
      );
    });
